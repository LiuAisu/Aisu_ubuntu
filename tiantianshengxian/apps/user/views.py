    ...
    def get(self, request, page_index):
        ...
        for order in orders:
            ...
            for order_sku in order_skus:
                ...
                # 动态给order_sku 加属性 保存订单商品小计
                order_sku.amount = amount
            # 给order添加属性 代表订单状态
            order.status_name = OrderInfo.ORDER_STATUS[order.order_status]
            # 动态给order加属性 保存订单商品信息
        ...
        order_page = paginator.page(page_index)
        # 在这里我们让每页最多显示5个页码
        # 总页数小于等于 5页
        ...
        contxet = {'order_page': order_page,
                   'pages': pages,
                   'page': 'order'}
        return render(request, 'dailyfresh/user/user_center_order.html', contxet)
